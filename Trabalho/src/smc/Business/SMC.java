...
public class SMC {
    ...
    public void showUtilizador(List<String> x, String n){
        Comum aux = this.comuns.get(n);
        for(String c : x)
            aux.adicionarMusica(c, this.conteudo.get(c).getCategoria());
        this.comuns.remove(n);
        this.comuns.put(n, aux);
    }
...
*/
    ...
    public void send(String nome, String amigo){
        Comum c = this.comuns.get(nome);
        c.adicionarPedidoAmizade(amigo);
        this.comuns.put(nome, c);
    }
